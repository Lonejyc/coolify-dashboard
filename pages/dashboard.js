...
import { Server, Cpu, ArrowUp, ArrowDown, Clock, Layers } from "lucide-react";
...
export async function getServerSideProps(context) {
  ...
  try {
    const agentUrl =
      process.env.MONITOR_AGENT_URL || "http://10.0.1.11:5000/api/stats";
...
function Sparkline({ data = [], color = "#10b981" }) {
  ...
  const pts = data.map(
    (v, i) => `${i * step},${H - (Math.min(Math.max(v, 0), 100) / 100) * H}`,
  );
            ...
            <div className="bg-black/40 backdrop-blur-md border border-zinc-800/50 rounded-xl p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-lg font-mono text-zinc-50 tabular-nums">
                  {info.uptime?.seconds != null
                    ? formatUptime(info.uptime.seconds)
                    : "—"}
                </p>
              </div>
            </div>
